...
const FetchLatest = async () => {
  const url =
    'https://listenapi.planetradio.co.uk/api9.2/audibles?StationCode=cl1&listenagain.ShowId%5B%5D=13035&_pp=15&premium=1';
  const snt_url =
    'https://listenapi.planetradio.co.uk/api9.2/audibles?StationCode=cl1&listenagain.ShowId%5B%5D=21729&_p=1&_pp=10&premium=1'; // sunday night takeover (George)
  ...
  try {
    ...
    const data = response.data
      ...
      .concat(
        snt_response.data.map((ep: Episode) => {
    ...
    for (const ep of data) {
      ...
      if (episode === null) {
        ...
        // download mp3 file
        const mp3_response = await axios.get(
          ep.listenagain.ScheduleListenAgainMP3Url,
          {
        ...
        const mp3_url = await uploadFile(
          filename,
          `gbx/${moment(ep.published_at).format('YYYY[/]MM[/]ddd_DD')}.mp3`,
        );
        ...
        const entry = await await db.execQueryOne(
          ...
          [
            ep.id,
            ep.title,
            ep.listenagain.ScheduleDescription,
            new Date(ep.published_at),
            ep.duration,
            mp3_url,
            ep.listenagain.ScheduleImageUrl,
            ep.type,
          ],
        );
        ...
        if (
          ...
          episode.description !== ep.listenagain.ScheduleDescription ||
          episode.duration !== ep.duration ||
          episode.image_url !== ep.listenagain.ScheduleImageUrl
        ) {
